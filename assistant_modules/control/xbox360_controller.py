# Xbox 360 controller module based on DualSense controller implementation
...
class Xbox360Controller:
    """
    Xbox 360 controller interface for rover control
    ...
    # Xbox 360 axis mappings
    AXIS_LEFT_X = 0
    AXIS_LEFT_Y = 1
    AXIS_RIGHT_X = 2
    AXIS_RIGHT_Y = 3
    AXIS_LEFT_TRIGGER = 4   # Left trigger (LT)
    AXIS_RIGHT_TRIGGER = 5  # Right trigger (RT)
    ...
    BUTTON_XBOX = 8         # Xbox guide button
    BUTTON_L3 = 9           # Left stick press
    BUTTON_R3 = 10          # Right stick press
    ...
    def process_buttons(self, current_time: float) -> bool:
        ...
        """
        # X button for lights
        x_button = self.joystick.get_button(self.BUTTON_X)
        if x_button and not self.last_x_button:
        ...
        # Y button for horn (haptic feedback)
        y_button = self.joystick.get_button(self.BUTTON_Y)
        if y_button and not self.last_y_button:
            if current_time - self.last_button_time > self.button_debounce:
                self._play_init_haptics()
        ...
        # Center gimbal on RB (Right Bumper) press
...
def test_controller():
    """Standalone test for Xbox 360 controller"""
    ...
    controller = Xbox360Controller(base)
    ...
    print("  Y button: Horn")
    print("  RB (Right Bumper): Center gimbal")
    print("  Start: Exit")
    print("=" * 40 + "\n")
